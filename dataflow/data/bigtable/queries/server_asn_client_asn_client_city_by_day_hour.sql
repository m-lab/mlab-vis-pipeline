...
FROM
  {0}
WHERE LENGTH(client_city) > 0 AND LENGTH(client_region_code) > 0 AND LENGTH(client_country_code) > 0 AND LENGTH(client_continent_code) > 0 AND LENGTH(client_asn_number) > 0 AND LENGTH(client_asn_name) > 0
