...
java -jar ${JAR_FILE} \
  ...
  --timePeriod="day" --project=mlab-oti --stagingLocation="gs://mlab-data-viz" \
  --skipNDTRead=0 --startDate=${STARTDATE} --endDate=${ENDDATE} --test=${TEST} &

echo "Running historic pipeline for HOUR"
java -jar ${JAR_FILE} \
  --runner=com.google.cloud.dataflow.sdk.runners.DataflowPipelineRunner \
  --timePeriod="hour" --project=mlab-oti --stagingLocation="gs://mlab-data-viz" \
  --skipNDTRead=0 --startDate=${STARTDATE} --endDate=${ENDDATE} --test=${TEST} &
...
java -cp ${JAR_FILE} mlab.bocoup.BigtableTransferPipeline \
  --runner=com.google.cloud.dataflow.sdk.runners.DataflowPipelineRunner \
  --project=mlab-oti --stagingLocation="gs://mlab-data-viz" --test=${TEST} \
  --diskSizeGb=15
