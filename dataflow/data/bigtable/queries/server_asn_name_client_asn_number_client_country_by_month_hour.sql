...
FROM
  {0}
WHERE LENGTH(client_country_code) > 0 AND LENGTH(client_continent_code) > 0 AND LENGTH(client_asn_number) > 0 AND LENGTH(client_asn_name) > 0 AND LENGTH(server_asn_name) > 0
